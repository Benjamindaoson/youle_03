  ...
  return (
    <aside className="flex h-screen w-[240px] flex-shrink-0 flex-col border-r border-wechat-line bg-white">
      ...
      <div className="flex items-center justify-between px-4 pb-1 pt-4">
        <span className="text-[17px] font-semibold text-[#111827]">消息</span>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/home/icon-new-chat.svg" alt="新建会话" width={20} height={20} className="cursor-pointer opacity-50 hover:opacity-80" title="新建会话" />
      </div>
      <div className="px-3 py-2">
        <label className="flex h-8 items-center gap-2 rounded-full bg-neutral-100 px-3">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-[12px] text-wechat-fg outline-none placeholder:text-neutral-400"
          />
        </label>
      </div>
...
function ChatRow({
  ...
  return (
    <button
      ...
      className={clsx(
        'flex h-14 w-full items-center gap-2.5 rounded-lg px-2 transition-colors',
        active ? 'bg-[#3B82F6]/[0.12]' : 'hover:bg-neutral-100',
      )}
      ...
      <div className="min-w-0 flex-1 text-left">
        <div className="mb-0.5 flex items-baseline justify-between gap-1">
          <span className="flex min-w-0 items-center gap-1">
            ...
            <span className="truncate text-[14px] text-[#111827]">{chat.name}</span>
            {chat.work_mode && (
        ...
        <div className="truncate text-[12px] text-[#6B7280]">{chat.preview ?? ''}</div>
      </div>
    </button>
  );
}
...
function ChatAvatar({ chat }: { chat: ConversationSummary }) {
  const SIZE = 'flex-shrink-0';
  const style: React.CSSProperties = { width: 34, height: 34 };
  if (chat.avatar_image) {
    /* eslint-disable-next-line @next/next/no-img-element */
    return <img src={chat.avatar_image} alt={chat.name} className={`${SIZE} rounded-full object-cover`} style={style} />;
  }
  if (chat.avatar_colors) {
    return (
      <div className={`${SIZE} grid grid-cols-2 gap-[1.5px] overflow-hidden rounded-full`} style={style}>
  ...
  return (
    <div
      className={`${SIZE} flex items-center justify-center rounded-full text-[12px] font-semibold text-white`}
      style={{ ...style, background: chat.avatar_bg ?? '#999' }}
    >
