...
**版本**:V2.0(对齐 v3.0 ADR 体系)
**日期**:2026-05-05
**面向**:工程团队(后端 / 前端 / AI 工程师 / DevOps / 技术负责人)入职**第 1 份必读**
**目的**:一份长文,读完后理解整个系统全貌,知道自己负责的模块怎么对接其他模块
**地位**:与 [启动配套/ARCHITECTURE.md](../启动配套/ARCHITECTURE.md) 是同样的"全栈视图",但本文更长更深,适合**第 2 周精读**;ARCHITECTURE.md 适合**第 1 天速览**。

> **v3.0 对齐说明**:本文已对齐 ADR-001-rev(Agent 编号反向:2=文档/3=图/4=影音)+ ADR-005-rev(音频归 Agent 4,队列 `agent_tasks:av`)+ ADR-013(7 角色)+ ADR-014(三模式)+ ADR-015(拟人化)。
> 若本文偶有"agent-video / agent_tasks:video",应统一读为 `agent-av / agent_tasks:av`。
> 若本文写"跨 Agent 调用",应理解为"主编排在 Skill YAML 显式拆步"(ADR-002)。
...
- **Agent 职责对应**:Agent 1 文字 / **Agent 2 文档** / **Agent 3 图** / **Agent 4 影音**(视频 + 音频)
...
- agent-av(Agent 4 影音):4 核 8G,2 实例
...
- agent_tasks:av           (Agent 4 影音,v3.0 ADR-005-rev)
...
| 4 个分任务 Agent | Agent 1 文字 / Agent 2 文档 / Agent 3 图 / Agent 4 影音(视频+音频) |
...
**决策日期**:2026-05-04
**决策**:Agent 2 = 文档 / Agent 3 = 图 / Agent 4 = 影音(视频+音频)
**理由**:
