...
**地位**:与 [模型路由表](../4_附录/模型路由表.md) 互补——本文偏"采购视角",路由表偏"运行时配置";信息冲突时**以本文为准**(因为含具体 V1.5 升级路径)。

> **v3.0 对齐说明**:本文已对齐 ADR-001-rev(Agent 编号反向)+ ADR-005-rev(音频归 Agent 4)+ ADR-013(7 角色)+ ADR-014(三模式)+ ADR-015(拟人化)。
> 若本文写"agent_tasks:video",应理解为 `agent_tasks:av`(队列名按 ADR-005-rev 统一)。
> 若本文出现"Agent 跨调用 / Agent 4 调 Agent 1",应理解为"由主编排在 Skill YAML 显式拆步"(ADR-002)。
...
- agent_tasks:av(Agent 4)        # v3.0 ADR-005-rev:音视频统一队列
- (HR / 财务经理不接队列,直接 API)
