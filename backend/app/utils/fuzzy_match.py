"""Fuzzy text find-and-replace for LLM-generated diffs.

Adapted from hermes-agent (MIT) © Nous Research — ``tools/fuzzy_match.py``.

Implements a multi-strategy matching chain to robustly find and replace
text, accommodating variations in whitespace, indentation, escaping, and
unicode-vs-ASCII drift common in LLM-generated edits.
...
Strategy chain (tried in order):
  ...
  6. trimmed_boundary       — trim whitespace on first/last line only
  7. unicode_normalized     — smart quotes, em/en dashes, ellipsis, NBSP
  8. block_anchor           — match first+last lines, similarity for middle
  9. context_aware          — 50% line-similarity threshold

Multi-occurrence is handled via the ``replace_all`` flag.
"""
...
def fuzzy_find_and_replace(
    ...
    Returns:
        ``(new_content, match_count, strategy_name, error)``.
        On success: ``(modified, n, strategy, None)``.
        On failure: ``(original, 0, None, error_msg)``.
    """
    if not old_string:
...
def _detect_escape_drift(
    ...
    from the matched region — indicates a transport layer added a stray
    backslash around an apostrophe/quote, and writing ``new_string``
    verbatim would corrupt the file.
    """
    ...
    for suspect in ("\\'", '\\"'):
        if (
            ...
            return (
                ...
                f"prefixed with a spurious backslash. Re-read the file and "
                f"pass old_string/new_string without backslash-escaping "
                f"{plain!r} characters."
            )
    return None
...
def find_closest_lines(
    ...
    """Return formatted "did you mean?" snippet showing the closest lines."""
...
def format_no_match_hint(
    ...
    """Append a 'Did you mean...' snippet for plain no-match errors only."""
    if match_count != 0:
        return ""
