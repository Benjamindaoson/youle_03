...
Adapted from hermes-agent (MIT) © Nous Research — ``agent/think_scrubber.py``.

A regex-based per-delta strip is correct for *complete* strings but wrong
when applied per-delta in a streamed output: it loses the open-tag state
across delta boundaries, so consumers downstream of the LLM (gateway WS,
agent_result_stream, persistence layer, TTS) see content that should have
stayed inside ``<think>``.

This module centralises the tag-suppression state machine so every
``stream_delta_callback`` sees text that has already had reasoning blocks
removed.  Partial tags at delta boundaries are held back until the next
delta resolves them; end-of-stream flushing surfaces any held-back prose
...
Re-entrant per agent instance.  Call ``reset()`` at the top of each new
turn so a hung block from an interrupted prior stream cannot taint the
next turn's output.
...
has been emitted on the current line.  This prevents prose that
*mentions* the tag name from being incorrectly suppressed.  Closed pairs
(``<think>X</think>``) are always suppressed regardless of boundary —
the model leaking inline reasoning is also worth removing.
"""
...
class StreamingThinkScrubber:
    ...
    def flush(self) -> str:
        """End-of-stream flush.  Discards held content if still in a block."""
        if self._in_block:
            self._buf = ""
            self._in_block = False
            return ""
